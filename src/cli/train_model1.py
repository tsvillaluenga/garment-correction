...
def main():
    ...
    for epoch in range(start_epoch, config['train']['epochs']):
        ...
        if epoch % config['val']['every_n_epochs'] == 0:
            ...
            if early_stopping is not None:
                if early_stopping(current_delta_e, epoch):
                    early_info = early_stopping.get_info()
                    progress_tracker.print_info(
                        f"🛑 Early stopping triggered! Best score: {early_info['best_score']:.4f} at epoch {early_info['best_epoch']}", 
                        "bold red"
                    )
                    break
        ...
        if scheduler_type == "reduce_on_plateau":
            scheduler.step(current_delta_e)  # Pass metric for plateau scheduler
        else:
    ...
    if early_stopping is not None and early_stopping.early_stop:
        ...
        progress_tracker.print_info(f"🏆 Best validation Delta E: {early_info['best_score']:.4f} at epoch {early_info['best_epoch']}", "bold cyan")
    else:
